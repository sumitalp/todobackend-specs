...
var url = process.env.URL || 'http://localhost:9000/todos';
...
describe('Cross Origin Requests', function(){
    ...
    it('show allow all origins', function(){
        return assert(result, "header").to.eventually.have.deep.property('access-control-allow-origin')
        .to.equal('*');
    })
});
...
describe('Create Todo Item', function() {
  ...
  it('should receive a location hyperlink', function() {
    return expect(result).to.eventually.have.deep.property('header')
  ...
  it('should create the item', function() {
    ...
    return assert(item, "body").to.eventually.have.deep.property("title")
    .that.equals('Walk the dog');
  });
...
describe('Update Todo Item', function() {
  ...
  it('should have completed set to true after PUT update', function() {
    ...
    return assert(result, "body").to.eventually.have.deep.property("completed")
    .to.be.true;
  });

  it('should have completed set to true after PATCH update', function() {
    var result = update(location, 'PATCH', {'completed': true});
    return assert(result, "body").to.eventually.have.deep.property("completed").to.be.true;
  });
